...
const split_ui = make_ui("split-ui", function (element, {
    ...
    function clamp_size(size) {
        const max_size = element.clientWidth - min_main_size;
        return Math.max(min_peripheral_size, Math.min(size, max_size));
    }

    function set_size(the_size) {
        size = clamp_size(the_size);
    ...
    const divider = dom("div", {
        ...
        onpointermove(event) {
            if (Number.isFinite(drag_origin)) {
                event.preventDefault();
                const the_size = clamp_size(drag_origin + (
                    placement === "bottom"
                    ? -event.pageY
                    : (
                        placement === "top"
    ...
    element.get_placement = get_placement;
