...
// This module exports the 'collapse' function, which replaces all occurrences
// of "import.meta.main" in the source code of a JavaScript module with "true"
// or "false". It also comments out any dead code that results, preserving line
// numbering.

//      collapse(text, main = false)

// This is an important optimization to apply to whole modules
// (https://james.diacono.com.au/whole_modules.html) because code and imports
// used only by the demo can delay loading significantly.
...
// Otherwise return undefined.
    ...
    if (node.type === "Literal") {
        ...
        if (node.operator === "&&") {
            if (boolify(node.left) === true && boolify(node.right) === true) {
                return true;
            }
            if (boolify(node.left) === false || boolify(node.right) === false) {
                return false;
            }
        ...
        const operand = boolify(node.argument);
...
function test_boolify() {
    ...
    Object.entries(cases).forEach(function ([text, expect]) {
        ...
        if (boolify(node) !== expect) {
            throw new Error("FAIL boolify " + text);
        }
    });
}
...
// make it unsafe to comment out lines with a double slash.
            ...
            const next_statement = statements[statement_nr + 1];
            if (next_statement !== undefined) {
                rx_crlf.lastIndex = statement.end;
                if (
                    !rx_crlf.test(text)
                    || rx_crlf.lastIndex > next_statement.start
                ) {
                    throw new Error(
                        "Missing newline after statement #" + statement_nr + "."
                    );
                }
            }
        }
...
function collapse(text, main = false) {
...
export default double;
`;
    const actual_true = collapse(text, true);
    if (actual_true !== expect_true) {
        throw new Error("FAIL collapse true" + actual_true);
    }
    const expect_false = `
// import test from "test";
...
export default double;
`;
    const actual_false = collapse(text, false);
    if (actual_false !== expect_false) {
        throw new Error("FAIL collapse false" + actual_false);
    }
}
...
export default Object.freeze(collapse);
