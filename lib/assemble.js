...
function one(make_node, message = "Unexpected.") {
    return function one_matcher(input) {
        if (input.token.kind === "end of file") {
...
function keyword(name) {
    return one(
        function predicate(token) {
            return (
                (token.kind === "name" && token.text === name)
                ? token
                : undefined
            );
        },
...
function parse(token_generator) {
    ...
    function make_input(position) {
        ...
        if (token?.kind === "error" && error_token === undefined) {
            error_token = token;
        }
        return {
            token,
