...
const error_bundle = {
    ...
    expected_b_a: "Expected {b} but saw {a}.",
...
// Skip over comments.
    ...
    function value(instruction_only, debug_label) {
        if (token.kind !== "space") {
...
// The label is an alias for the following label.
        ...
        define_object[name_token.text] = value(
            instruction_only,
            name_token.text
        );
...
// The local ref instruction check, along with its test cases, has been disabled
// to permit the inclusion of ad hoc instruction sequences as data. This is a
// convenience for exercising the machine with invalid instructions.
...
function test_parse() {
    ...
    function outdent(text) {
        return text.replace(rx_indent, "");
    }

    function good(description, text) {
        claims.push(function good_validator() {
            const result = parse(tokenize(outdent(text)));
            return (
    ...
    `);
    const common_tails = parse(tokenize(outdent(`
        a:
            pair_t #t
        b:
            pair_t #f #nil
...
// globalThis.console.log(JSON.stringify(assemble(`
// boot:
//     actor self
//     end commit
// .export
