...
function decompose_number(number) {
    ...
    if (Number.isFinite(number) && (number !== 0)) {
        ...
        while (integer % 2 === 0 && exponent !== 0) {  // push information into exponent
            integer /= 2;
            exponent += 1;
        }
    }
...
function decode_natual(octets, offset) {
    let number = decode_integer(octets, offset);
    if (number.error) return number;  // report error
    if (number.value >= 0) {
        return number;
    }
    return { error: "natural number required", octets, offset, value: number.value };
}
...
*/
    ...
    if (prefix === rational_octet) {
        base = decode_natual(octets, offset);
    ...
    const size = decode_natual(octets, offset);
...
*/
    ...
    if (prefix === string_octet) {
        ...
        const size = decode_natual(octets, meta.offset);
        if (size.error) return size;  // report error
...
function decode_blob(octets, offset) {
    ...
    if (prefix === blob_octet) {
        // raw octet sequence
        const size = decode_natual(octets, offset + 1);
        if (size.error) return size;  // report error
        offset = size.offset + size.value;
