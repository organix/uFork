...
//debug function good(humus_text, expected_prints) {
//debug     return function good_requestor(callback) {
//debug         try {
//debug             const ir = compile(humus_text);
//debug             if (ir.errors.length > 0) {
//debug                 return callback({
//debug                     pass: false,
//debug                     detail: ir.errors.map(function (error) {
//debug                         return "  - " + error.message;
//debug                     }).join("\n"),
//debug                     humus_text
//debug                 });
//debug             }
//debug             if (expected_prints === undefined) {
//debug                 return callback({
//debug                     pass: true,
//debug                     humus_text
//debug                 });
...
//debug             }
//debug             let cancel;
//debug             let timer = setTimeout(function () {
//debug                 cancel();
//debug                 return callback({
//debug                     pass: false,
...
//debug                 });
//debug             }, 1000);
//debug             cancel = parseq.parallel(
//debug                 [generate_debug_url(humus_text)],
//debug                 [run(humus_text)],
//debug                 undefined,
//debug                 true
//debug             )(function (value, reason) {
//debug                 clearTimeout(timer);
//debug                 if (value === undefined) {
//debug                     return callback(undefined, reason);
//debug                 }
//debug                 const [debug_url, run_report] = value;
//debug                 if (
//debug                     run_report === undefined
//debug                     || run_report.prints.length !== expected_prints.length
//debug                     || !run_report.prints.every(function (print, print_nr) {
//debug                         return print.endsWith(expected_prints[print_nr]);
//debug                     })
//debug                 ) {
//debug                     let lines = [];
//debug                     if (run_report !== undefined) {
//debug                         run_report.logs.forEach(function (entry) {
//debug                             lines.push(["  -", ...entry].join(" "));
//debug                         });
//debug                     }
//debug                     lines.push("  --> " + debug_url);
//debug                     return callback({
//debug                         pass: false,
//debug                         detail: lines.join("\n"),
//debug                         humus_text
//debug                     });
//debug                 }
//debug                 return callback({
...
//debug             return callback(undefined, exception);
//debug         }
//debug     };
//debug }

//debug function bad(humus_text) {
//debug     return function bad_requestor(callback) {
//debug         try {
//debug             const ir = compile(humus_text);
//debug             return callback(
//debug                 ir.errors.length > 0
//debug                 ? {pass: true, humus_text}
//debug                 : {pass: false, humus_text}
//debug             );
//debug         } catch (exception) {
//debug             return callback(undefined, exception);
//debug         }
...
//debug const claims = {
//debug     "empty": good("", []),
//debug     "constant": good("TRUE", []),
//debug     "? constant": good("SEND ? TO println", "#?"),
//debug     "unknown constant": bad("MAYBE"),
//debug     "print constant": good("SEND 42 TO println", "+42"),
//debug     "nested primitive calls": good(`
//debug         SEND sub(mul(2, 3), not(4)) TO println
//debug     `, "#?"),
//debug     "pair expression": good(`
//debug         SEND 1, 2, 3 TO println
//debug     `, "(+1 +2 . +3)"),
//debug     "capture non-existant variable": bad("DEF oops AS neg(x)"),
//debug     "circular reference": bad("DEF x AS x"),
//debug     "define constants": good(`
//debug         DEF x AS 666
//debug         DEF y AS 777
//debug         SEND x, y TO println
//debug     `, "(+666 . +777)"),
//debug     "cyclic reference": bad(`
//debug         DEF a AS b
//debug         DEF b AS a
//debug     `),
//debug     "forward reference": good(`
...
//debug         DEF a AS λy.y
//debug     `, "+42"),
//debug     "destructured definition": good(`
//debug         DEF (a, b), c AS (1, 2), 3
//debug         SEND a, b, c TO println
//debug     `, "(+1 +2 . +3)"),
//debug     "dereference _": bad(`
//debug         DEF _ AS 1
//debug         SEND _ TO println
//debug     `),
//debug     "define unbound": good("DEF _ AS 1"),
//debug     "define constant match": good(`
//debug         DEF 1 AS 1
//debug         SEND 42 TO println
//debug     `, "+42"),
//debug     "define constant mismatch": good(`
//debug         DEF 1 AS 2
//debug         SEND 42 TO println
//debug     `, []),
//debug     "forward a message": good(`
...
//debug     `, "+42"),
//debug     "capture value in closure": good(`
//debug         DEF x AS 3
//debug         DEF triple(n) AS mul(x, n)
//debug         SEND triple(2) TO println
//debug     `, "+6"),
//debug     "calling returned closures": good(`
//debug         DEF hof3 AS λa.λ(b, c).λd.(a, b, c, d)
//debug         SEND ((hof3(1))(2, 3))(4) TO println
//debug     `, "(+1 +2 +3 . +4)"),
//debug     "recursion": good(`
//debug         DEF loop(a) AS loop(a)
//debug         loop(42)
//debug     `),
//debug     "closure self-reference shadowed by inner variable": good(`
//debug         DEF x AS λx.x
//debug         SEND x(42) TO println
//debug     `, "+42"),
//debug     "closure self-reference shadows outer variable": good(`
...
//debug     `, "+720"),
//debug     "recursive tail call": good(`
...
//debug     `, "+8"),
//debug     "symbol equality": good(`
//debug         SEND and(eq(#foo, #foo), not(eq(#foo, #bar))) TO println
//debug     `, "#t"),
//debug     "shadow primitive": good(`
...
//debug     `, "(-1 +0 +1 . #?)"),
//debug     "redefinition": bad(`
//debug         DEF x AS 1
//debug         DEF x AS 2
//debug     `),
//debug     "redefinition within DEF pattern": bad(`
//debug         DEF x, x AS 1, 2
//debug     `),
//debug     "redefinition within parameters": bad(`
//debug         DEF foo(a, b, a) AS 1
//debug     `),
//debug     "redefinition within CASE pattern": bad(`
//debug         CASE 123 OF
//debug         x, x : 1
//debug         END
//debug     `),
//debug     "redefinition of CREATE": bad(`
//debug         DEF a AS 1
...
//debug     `, []),
//debug     "DEF pattern failure": good(`
//debug         SEND TRUE TO println  # never sent
...
//debug     `, "((+3 +4 . +5) +2 . +1)"),
//debug     "bad equation": bad("LET x = y"),
//debug     "IF pattern match": good(`
//debug         DEF y AS 666
//debug         IF x = 777 [
...
//debug         ]
//debug     `, "(+666 . +777)"),
//debug     "IF pattern mismatch": good(`
//debug         DEF x AS 111
...
//debug function print_report(report, log_level, description) {
//debug     if (log_level >= ufork.LOG_TRACE) {
//debug         const ir = compile(report.humus_text);
//debug         globalThis.console.log(JSON.stringify(ir, undefined, 4));
//debug         globalThis.console.log(disassemble(ir));
//debug     }
//debug     if (report.pass) {
//debug         if (log_level >= ufork.LOG_WARN) {
//debug             globalThis.console.log("OK", description);
//debug         }
//debug     } else {
//debug         globalThis.console.log("FAIL", description);
//debug     }
//debug     if (log_level >= ufork.LOG_DEBUG && report.detail !== undefined) {
//debug         globalThis.console.log(report.detail);
//debug     }
//debug }

//debug function test_compile(log_level) {
//debug     parseq.parallel(
//debug         Object.values(claims)
//debug     )(
//debug         function callback(reports, reason) {
//debug             if (reports === undefined) {
//debug                 throw reason;
//debug             }
//debug             reports.forEach(function (report, report_nr) {
//debug                 const description = Object.keys(claims)[report_nr];
//debug                 print_report(report, log_level, description);
//debug             });
//debug             if (reports.every((report) => report.pass)) {
//debug                 globalThis.console.log("All tests passed!");
//debug             } else {
//debug                 globalThis.reportError("FAIL test_compile");
//debug             }
//debug         }
//debug     );
//debug }
...
//debug test_search();
//debug test_compile(ufork.LOG_INFO); // LOG_INFO | LOG_WARN | LOG_DEBUG
