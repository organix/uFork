...
//debug         tail: {kind: "ident_ptrn", ident: "d"}
//debug     }
//debug };
//debug search(search_test_ptrn, "a");
//debug search(search_test_ptrn, "b");
//debug search(search_test_ptrn, "c");
//debug search(search_test_ptrn, "d");
...
// identifier if it has one.
        ...
        const def_ptrn = def_ptrns[0];
        const name = def_ptrn?.ident ?? "?";
        const label = "$" + Object.keys(definitions).length + ":" + name;
        definitions[label] = undefined;  // placeholder

// Compile the closure's code.

        def_abs_ptrns.unshift(def_ptrn);
        abs_labels.unshift(
            def_ptrn?.kind === "ident_ptrn"
            ? label
            : undefined
        );
        push_env(ast.ptrn, 0);
...
// Search the environment for a match.
        ...
        const nth_imms = search(env_ptrn, ast.ident);
        const abs_env_nr = def_abs_ptrns.findIndex(function (ptrn) {
            return ptrn?.ident === ast.ident;
        });

// Check if a closure has referenced itself by name. Due to the immutability of
// quads, a circular reference is impossible. The best we can do is create an
// equivalent closure on demand.

        if (
            abs_env_nr !== -1                   // closure self-reference found
            && (
                nth_imms === undefined          // variable not found
                || abs_env_nr + 1 < nth_imms[0] // variable found but shadowed
            )
        ) {
            const label = abs_labels[abs_env_nr];
            const make_closure = ir_ref("make_closure", "$hum");
            k =                                         // env ...
                ir_instr(ast, "push", ir_ref(label),    // env ... code env
                ir_instr(ast, "pick", env_at,           // env ... code env
                ir_instr(ast, "nth", -abs_env_nr - 1,   // env ... code abs_env
...
// Retrieve the value from the environment.
        ...
        nth_imms.reverse().forEach(function (nth_imm) {
            k = ir_instr(ast, "nth", nth_imm,
                k);
        });
        k =                                             // env ...
...
// identifiers.
        ...
        elements.forEach(function (element, element_nr) {
            def_ptrns[0] = sub_ptrns[element_nr];
            k = generate(element, k);
        });
        def_ptrns[0] = def_ptrn;
        return k;
    }
...
//debug     DEF k0 AS \\a.\\b.\\c.(a, b, c)
//debug     DEF k1 AS k0(1000)
//debug     SEND k1 TO println
//debug     DEF k2 AS k1(1001)
//debug     SEND k2 TO println
//debug     DEF k3 AS k2(1002)
//debug     SEND k3 TO println
//debug `;
