...
function parse(text) {
    ...
    try {
        ...
        return {
            lang: "Humus",
            ast: unflatten("stmt_pair", prog.concat({kind: "empty_stmt"})),
            tokens,
            errors: []
        };
    } catch (exception) {
...
function analyze_idents(stmt) {
    ...
    function scoped(visitor) {
        return function scoped_visitor(ast, visitors) {
            enter_scope();
    ...
    visit(stmt, Object.assign({}, default_visitors, {
        ...
        abs_expr: scoped(default_visitors.abs_expr),
        case_choice: scoped(default_visitors.case_choice)
    }));
    return exit_scope();
}

function provides(define, free) {
    return Object.keys(define).filter(function (ident) {
        return free[ident] !== undefined;
    });
}

function sort_stmts(stmt_array) {

// Attempt to remove forward references by performing a topological sort on an
// array of statements.

    return stmt_array.map(function (ast) {
        const {define, free} = analyze_idents(ast);
        return {ast, define, free};
    }).sort(function (a, b) {
        const ba = provides(b.define, a.free);
        const ab = provides(a.define, b.free);
        return ba.length - ab.length;
    }).map(function (object) {
        return object.ast;
    });
}
...
//debug const text = `
//debug     SEND b(y) TO println
//debug     DEF b AS λ_.y
//debug     DEF y AS a(42)
//debug     DEF a AS λy.y
//debug `;
//debug sort_stmts(flatten(parse(text).ast)).map(function (stmt) {
...
function codegen(crlf, src) {
    ...
    const compare = ir_ref("compare", "$hum");
...
// when one branch is known to abort.
    ...
    function gen_block_code(ast) {
        ast = unflatten("stmt_pair", sort_stmts(flatten(ast)));
    ...
    const primitives = {
        ...
        compare(ast, k) {
            k =                                         // args
                ...
                ir_instr(ast, "push", compare,          // args k compare
                ir_call(ast, 1, 1)));
            return k;
        },
        neg(ast, k) {
...
// Inline calls to primitives.
        ...
        if (
            ...
            && search(env, ast.abs.ident) === undefined
        ) {
...
// (such as 'println') in its environment.
    ...
    push_env({
        kind: "pair_ptrn",
        head: {kind: "ident_ptrn", ident: "random"},
        tail: {
            kind: "pair_ptrn",
            head: {kind: "ident_ptrn", ident: "timer"},
            tail: {kind: "ident_ptrn", ident: "println"}
        }
    });
...
//debug     DEF x AS 2
//debug `);
//debug claim(bad, "duplicate parameter", `
