...
//debug //console.log(depths, op, imm?.kind ?? imm, `(${delta})`);
    ...
    function ir_if_instr(gen_t, gen_f) {
...
// autogenerated labels to avoid naming conflicts.
    ...
    function gen_abs_expr(ast, k) {
        depths.unshift(0);
        env_unshift(ast.ptrn);
        const code =                                    // args k env
            as(ast, ir_instr("roll", 3,                 // k env args
            as(ast, ir_instr("pair", 1,                 // k env'=(args . env)
            generate(ast.body,                          // k env' rv
            as(ast, ir_instr("roll", -3,                // rv k env'
            as(ast, ir_instr("drop", 1,                 // rv k
            as(ast, ir_instr("jump")))))))))));         // rv
        env_shift();
        depths.shift();
        const make_closure = ir_ref("make_closure", "$hum");
        depths[0] -= 1 - 3;  // ( code env k -- closure )
        k =                                             // env ...
            as(ast, ir_instr("push", code,              // env ... code
            as(ast, ir_instr("pick", env_at,            // env ... code env
            as(ast, ir_instr("push", k,                 // env ... code env k
            make_closure))))));
        return k;
    }

    function check_arity(ptrn, expected_arity) {
        const arity = flatten(ptrn).length;
        if (arity === expected_arity) {
            return true;
        }
        error("wrong_arity", ptrn, expected_arity, arity);
        return false;
    }

    function push_false(ast, k) {
        return function gen_push_false() {
            return as(ast, ir_instr("push", ir_lit("false"), k));
        };
    }

    function push_true(ast, k) {
        return function gen_push_true() {
            return as(ast, ir_instr("push", ir_lit("true"), k));
        };
    }

    function binary_op(op, imm) {
        return function gen_binary_op(ast, k) {
            if (check_arity(ast.arg, 2)) {
...
// first interned, avoiding duplication in the IR.
    ...
    function gen_app_expr(ast, k) {
        if (
            ...
            const gen = primitives[ast.abs.ident];
            return gen(ast, k);
        }

// TODO allow primitives to be shadowed

        depths[0] -= 1 - 2;  // ( args k -- rv )
        k =
            generate(ast.arg,                           // args
            as(ast, ir_instr("push", k,                 // args k
            generate(ast.abs,                           // args k closure
            as(ast, ir_instr("jump"))))));              // args k
        return k;
    }
...
//      nth 4      nth -4
        ...
        k =                                             // env ...
            as(ast, ir_instr("pick", env_at,            // env ... env
            k));                                        // env ... value
        return k;
    }
...
// the AST, building the instruction sequence in reverse. The definition,
// earlier added to the static environment by 'gen_stmt_pair', is inaccessible
...
// statements.
        ...
        k =                                             // env ...
            as(ast, ir_instr("pick", env_at,            // env ... env
            as(ast, ir_instr("part", 1,                 // env ... rest scope
            generate(ast.expr,                          // env ... rest scope value
            as(ast, ir_instr("pair", 1,                 // env ... rest (value . scope)
            as(ast, ir_instr("pair", 1,                 // env ... env'=((value . scope) . rest)
            as(ast, ir_instr("roll", env_at,            // ... env' env
            as(ast, ir_instr("drop", 1,                 // ... env'
            as(ast, ir_instr("roll", () => -env_at(),   // env' ...
            k)))))))))))))));
        return k;
    }
...
// (such as 'println') in its environment.
    ...
    depths.unshift(0);  // never used
    env_unshift({kind: "ident_ptrn", ident: "println"});
    definitions.boot =
