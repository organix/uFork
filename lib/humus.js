...
// autogenerated labels to avoid naming conflicts.
    ...
    function binary_op(op, imm) {
        return function gen_binary_op(ast, k) {
            k = generate(ast.arg,                       // (n . m)
                ir_instr(ast.arg, "part", 1,            // m n
                ir_instr(ast.arg, "roll", 2,            // n m
    ...
    const primitives = {
        ...
        not(ast, k) {
            ...
            k = generate(ast.arg,
                ir_if_instr(
                    ast,
                    () => ir_instr(ast, "push", ir_lit("false"), k),
                    () => ir_instr(ast, "push", ir_lit("true"), k)
                ));
            return k;
        },
...
// first interned, avoiding duplication in the IR.
        ...
        and(ast, k) {
            k = intern(k);
            k = generate(ast.arg,                                           // args=(a . b)
                ir_instr(ast.arg, "part", 1,                                // b a
                ir_if_instr(                                                // b
                    ast,
                    () =>
                        ir_if_instr(                                        // --
