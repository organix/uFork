...
// Minimize forward references in a sequence of statements by performing a
// topological sort.
    ...
    return stmt_array.map(function (ast) {
        ...
        const nr_a_uses = provides(b.define, a.free).length;
        const nr_b_uses = provides(a.define, b.free).length;
        return nr_a_uses - nr_b_uses;
    }).map(function (object) {
        return object.ast;
    });
}
...
// environment must initially be extended with every definition in the block.
        ...
        flatten(ast).forEach(function (stmt) {
            if (stmt.kind === "def_stmt") {
                ...
                env.head = {
                    kind: "pair_ptrn",
                    head: {kind: "ident_ptrn", ident: stmt.ident},
                    tail: env.head
                };
            }
        });
