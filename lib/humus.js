...
const empty_env = {kind: "ident_ptrn", ident: "_"};
const error_bundle = {
...
// first interned, avoiding duplication in the IR.
    ...
    function gen_ident_expr(ast, k) {
        let scope_nth;
        const env_nr = envs.findIndex(function (ptrn) {
            scope_nth = 0;
            while (ptrn.kind === "pair_ptrn") {
                scope_nth += 1;
                if (ptrn.head.kind !== "ident_ptrn") {
                    error("not_implemented", ptrn.head, "only ident ptrns");
                    return false;
                }
                if (ptrn.head.ident === ast.ident) {
                    return true;
                }
                ptrn = ptrn.tail;
            }
            ...
            if (ptrn.kind !== "ident_ptrn") {
                error("not_implemented", ptrn, "only ident ptrns");
            }
        ...
        k =                                             // env ...
            ...
            as(ast, ir_instr("nth", env_nr + 1,         // env ... scope
            as(ast, ir_instr("nth", scope_nth,          // env ... value
            k))))));
        return k;
    }
    ...
    function gen_def_stmt(ast, k) {
        envs[0] = envs[0].tail;  // remove from scope
        k =                                             // env ...
...
// The lexical environment is extended on the way down.
// The dynamic environment is extended on the way up.
...
// (such as 'println') in its environment.
    ...
    envs.unshift({kind: "ident_ptrn", ident: "println"});
    definitions.boot =
    ...
    if (envs.length !== 1 || depths.length !== 1) {
        throw new Error("Unexpected envs or depths length.");
    }
    return {
        lang: "uFork",
        ast: {
