...
// first interned, avoiding duplication in the IR.
    ...
    function gen_const_expr(ast, k) {
        ...
        if (Number.isSafeInteger(value)) {
            k = as(ast,
                ir_instr("push", value,                 // ... value
                k));
        } else if (value === true) {
            k = as(ast,
                ir_instr("push", ir_lit("true"),        // ... #t
                k));
        } else if (value === false) {
            k = as(ast,
                ir_instr("push", ir_lit("false"),       // ... #f
                k));
        } else {
...
// Top-level `println` is implemented by the debug device.
        ...
        k = as(ast, ir_instr("msg", 0,                  // ... {caps}
            as(ast, ir_instr("push", debug_key,         // ... {caps} dev.debug_key
            as(ast, ir_instr("dict", "get",             // ... debug_dev
            k))))));
        return k;
    }
...
//debug     //console.log(disassemble(result));
//debug }
