...
// Remember that we are working our way up, building the instruction sequence in
// reverse. Earlier statements (yet to be generated) have no access to this
// definition without forward reference, so we discard it from the lexical
// environment (to which it was added on the way down).

        env_ptrn.head = env_ptrn.head.tail;

// Then we extend the dynamic environment with the defined value.
