...
function ir_const(value) {
    ...
    throw new Error("Not implemented ir_const " + value);
}
...
// of each branch. Our invariant holds when they are the same.
    ...
    function gen_block_code(body) {
        ...
        const code =                                    // k env
            ir_instr(body, "push", ir_undef,            // k env #?
    ...
    function gen_ptrn_match(ast, k) {
        ...
        if (ast.kind === "const_ptrn") {
            k =                                             // value
                ir_instr(ast, "eq", ir_const(ast.value),    // value==const
                k);
            return k;
        }
        ...
        throw new Error("Not implemented gen_ptrn_match " + ast.kind);
    }
...
// Otherwise obtain the procedure (usually a closure) as a value and call it.
    ...
    function gen_const_expr(ast, k) {
        k = ir_instr(ast, "push", ir_const(ast.value),  // value
            k);
        return k;
    }
