...
function is_evaluable(expr) {
    return (
        expr.kind === "abs_expr"
        || expr.kind === "const_expr"
        || expr.kind === "ident_expr"
        || (
            expr.kind === "pair_expr"
            && is_evaluable(expr.head)
            && is_evaluable(expr.tail)
        )
    );
}

function is_definable(stmt) {
    return (
        stmt.kind === "def_stmt"
        && stmt.ptrn.kind === "ident_ptrn"
        && is_evaluable(stmt.expr)
    );
}

function test_is_definable() {
    const exports = flatten(parse(`
        DEF a AS 1
        DEF a AS b # assumes b is definable!
        DEF a AS b,c,d,?,1,(e,f),NIL
        DEF a() AS 1
        DEF a() AS NEW b
        DEF a() AS []
        DEF a AS (λa.a),(λa.a)
    `).ast);
    const boot = flatten(parse(`
        DEF a AS []
        DEF a AS b,(NEW c),d
        DEF a AS b,c()
        DEF a AS b()
        ...
        a()
    `).ast);
    if (!exports.every(is_definable) || boot.some(is_definable)) {
        throw new Error("FAIL is_definable");
    }
}
...
function search(ptrn, ident) {
    if (ptrn.abs_idents !== undefined && ptrn.abs_idents.includes(ident)) {
        return [];  // found closure env
    }
...
function test_search() {
    ...
    ["a", "b", "c"].forEach(function (ident) {
        const path = search(test_ptrn, ident);
        const ptrn = path.reduce(nth, test_ptrn);
        if (ident !== ptrn.ident) {
            throw new Error("FAIL search " + ident);
        }
    });
    if (search(test_ptrn, "d").join() !== "-1") {
        throw new Error("FAIL search d");
    }
}

function is_tail(path) {
...
function decompose_path(path) {
    if (path.length > 0) {
        if (path[0] > 1) {
            return [-1, path[0] - 1, ...path.slice(1)];
        }
        if (path[0] < -1) {
            return [-1, path[0] + 1, ...path.slice(1)];
        }
...
function path_contains(a, b, exclusive = false) {

// Returns true if the value at path 'a' contains the value at path 'b'.

// If 'a' is the entire value, it must contain 'b' unless we are being exclusive
// and 'b' is also the entire value.

    if (a.length === 0) {
        return !exclusive || b.length > 0;
    }
    const aa = decompose_path(a);
    if (aa !== undefined) {
        return path_contains(aa, b, exclusive);
    }
    const bb = decompose_path(b);
    if (bb !== undefined) {
        return path_contains(a, bb, exclusive);
    }
    if (a[0] !== b[0]) {
        return false;
    }
    return path_contains(a.slice(1), b.slice(1), exclusive);
}
...
function test_path_contains() {
    if (
        ...
        || !path_contains([], [-1])
        || path_contains([-1], [])
        || !path_contains([3, 2, 1], [3, 2, 1])
        || !path_contains([3, 2], [3, 2, 1])
        || path_contains([3, 2, 1], [3, 2])
        || path_contains([2], [3])
        || path_contains([3], [2])
        || !path_contains([-2], [-3])
        || path_contains([-3], [-2])
        ...
        throw new Error("FAIL path_contains");
    }
}

function compact(env, idents) {

// Return an array of paths that can be used to reconstruct 'env' such that it
// contains only the information necessary to resolve the 'idents'.

// Variable access time is reduced due to the environment tree being flattened
// into a list, but both time and space is required to construct said list so
// this may not always be beneficial to performance.

// More importantly, unused portions of the environment are made inaccessible.
// This can significantly simplify the actor reference graph of a running
// program, wherein ambient capabilities like 'println' would otherwise be
// referenced by every single actor regardless of whether it was used.

    const ident_paths = idents.map(function (ident) {
        return search(env, ident);
    }).filter(function found(path) {
        return path !== undefined;
    });

// Inherit the environment of the nearest parent closure referenced by a free
// variable, or the empty environment if no closures are referenced. This is
// necessary to facilitate recursion and self-reference. Then prepend each free
// variable that is not present in the inherited environment. Consider the
// compilation of the following closures, ignoring the blocks for now.

//                                          ARGS    FREE        ENV
//  DEF a(p) AS [                           p       x           a_env=x,_
//      DEF b(q) AS [                       q       x a         b_env=a_env
//          DEF c(r) AS [                   r       x a b q     c_env=q,b_env
//              DEF d(s) AS x(a, b, q, r)   s       x a b q r   d_env=r,q,b_env
//          ]
//      ]
//  ]

    let inherited;
    ident_paths.forEach(function (path) {
        if (
            is_tail(path)
            && (inherited === undefined || path_contains(path, inherited))
        ) {
            inherited = path;
        }
    });
    return ident_paths.filter(function (path) {
        return inherited === undefined || !path_contains(inherited, path);
    }).concat(
        [inherited]
    );
}
...
function test_compact() {
    const env = {
        ...
        head: {
            kind: "pair_ptrn",
            head: {kind: "ident_ptrn", ident: "p"},
            tail: {kind: "ident_ptrn", ident: "q"}
        },
        tail: {
            kind: "pair_ptrn",
            head: {kind: "ident_ptrn", ident: "r"},
            tail: {
                kind: "pair_ptrn",
                head: {kind: "ident_ptrn", ident: "s"},
                tail: {kind: "ident_ptrn", ident: "_"},
                abs_idents: ["c"]
            },
            abs_idents: ["b"]
    ...
    if (
        JSON.stringify(compact(env, ["x"])) !== "[null]"
        || JSON.stringify(compact(env, ["a"])) !== "[[]]"
        || JSON.stringify(compact(env, ["p"])) !== "[[1,1],null]"
        || JSON.stringify(compact(env, ["a", "c", "p"])) !== "[[]]"
        || JSON.stringify(compact(env, ["q", "b", "c"])) !== "[[1,-1],[-1]]"
    ) {
        throw new Error("FAIL compact");
    }
}
...
const error_bundle = {
    ...
    expected_a_b: "Expected {a} but saw {b}.",
...
function codegen(crlf, src) {
    let counter = -1;
    let def_stmts = Object.create(null);
    ...
    const ir_actor_t = ir_type("actor");
...
// Enable the following fragment to debug stack depth inconsistencies.
        ...
        if (false) {
            ...
            globalThis.console.log(depths(env), op, imm_summary, `(${delta})`);
        }
    }
...
// when one branch is known to abort.
    ...
    function ir_nth_instrs(debug_ast, path, k) {
        return path.reduceRight(function (k, imm) {
            return ir_instr(debug_ast, "nth", imm, k);
        }, k);
    }
    ...
    function ir_compact(debug_ast, paths, k) {
        k =
            ir_instr(debug_ast, "pair", paths.length - 1,   // env env'
            ir_instr(debug_ast, "roll", -2,                 // env' env
            ir_instr(debug_ast, "drop", 1,                  // env'
            k)));
        paths.forEach(function (path) {
            if (path === undefined) {
                k =                                         // env ...
                    ir_instr(debug_ast, "push", ir_undef,   // env ... _
                    k);
            } else {
                k =
                    ir_instr(debug_ast, "pick", env_at,     // env ... env
                    ir_nth_instrs(debug_ast, path,          // env ... value
                    k));
            }
        });
        return k;
    ...
    function gen_ptrn_match(ast, k) {
        ...
        if (ast.kind === "value_ptrn") {
            k =                                         // value
                ...
                ir_instr(ast.expr, "push", k,           // value $value k
                ir_instr(ast.expr, "push", eq,          // value $value k eq
                ir_call(ast.expr, 2, 1))));             // match
            return k;
        }
...
// Determine what parts of the environment must be kept to satisfy the closure's
// free variables.

        const free = Object.keys(analyze_idents(ast).free);
        const paths = compact(env, free);
        const previous_env = env;
        env = unflatten("pair_ptrn", paths.map(function (path) {
            return (
                path !== undefined
                ? path.reduce(nth, env)
                : {kind: "any_ptrn"} // empty env
            );
        }));
        if (def_ptrn?.kind === "ident_ptrn") {

// It is possible for an environment to be shared by multiple nested closures.
// If that is the case, make a shallow copy so that the shared 'abs_idents'
// array is not modified.

            if (env.abs_idents !== undefined) {
                env = Object.assign({}, env); // avoid mutating abs_idents
            } else {
                env.abs_idents = [];
            }
            env.abs_idents.push(def_ptrn.ident);
            env.abs_label = label;
        }
        env = {
            kind: "pair_ptrn",
...
// A closure is constructed from its compiled code and its environment.
        ...
        k =                                             // env ...
            ...
            ir_compact(ast.body, paths,                 // env ... code env'
...
// dynamic creation of a closure and jump straight to the code.
            ...
            if (ast.abs.kind === "ident_expr") {
                const path = search(env, ast.abs.ident);
                if (path !== undefined && is_tail(path)) {
                    const label = path.reduce(nth, env).abs_label;
                    if (label !== undefined) {
                        const code = ir_ref(label);
                        k =                             // k env
                            ir_instr(ast, "dup", 1,     // k env env
                            ir_nth_instrs(ast, path,    // k env env'
...
// Search the environment for a match.
        ...
        const path = search(env, ast.ident);
        if (path === undefined) {
            if (def_stmts[ast.ident] !== undefined) {
                k =
                    ir_instr(ast, "push", ir_ref(ast.ident),        // definition
                    k);
                return k;
            }
        ...
        if (is_tail(path)) {
            ...
            if (label !== undefined) {
                k =                                         // env ...
                    ...
                    ir_nth_instrs(ast, path,                // env ... code env'
                    ir_instr(ast, "push", k,                // env ... code env' k
                    ir_instr(ast, "push", make_closure,     // env ... code env' k make_closure
...
// Retrieve the value from the environment.
        ...
        k =                                             // env ...
            ir_instr(ast, "pick", env_at,               // env ... env
            ir_nth_instrs(ast, path,                    // env ... value
            k));
        return k;
    }
...
// A LET statement is rewritten to its equivalent DEF AS statement.
    ...
    function gen_send_stmt(ast, k) {
        k = generate(ast.msg,                           // msg
            ...
            ir_instr(ast, "dup", 1,                     // msg actor actor
            ir_instr(ast, "typeq", ir_actor_t,          // msg actor cap?
            ir_if_instr(
                ast,
                () =>                                   // msg actor
    ...
    function eval_expr(ast) {
        if (ast.kind === "const_expr") {
            return ir_const(ast);
        }
        if (ast.kind === "ident_expr") {
            if (def_stmts[ast.ident] !== undefined) {
                return ir_ref(ast.ident);
            }
            if (Object.hasOwn(primitives, ast.ident)) {
                return primitives[ast.ident];
            }
            return error("undefined_a", ast, ast.ident);
        }
        if (ast.kind === "abs_expr") {

// Top level functions carry no environment, so they can be compiled as simple
// assembly procedures.

            redefinition_check(ast.ptrn);
            const previous_env = env;
            env = Object.assign({depth: 2}, ast.ptrn);
            const procedure =                               // args k
                ir_instr(ast.body, "roll", 2,               // k args
                ir_instr(ast.body, "dup", 1,                // k env=args args
                gen_ptrn_match(ast.ptrn,                    // k env match?
                ir_if_instr(                                // k env
                    ast.body,
                    () =>
        ...
        if (ast.kind === "pair_expr") {
            return ir_pair(
                eval_expr(ast.head),
                eval_expr(ast.tail)
            );
        }
        throw new Error("Unevaluable expression.");
    }
...
// Top level DEF statements that are pure can be compiled as module definitions,
// but others must be compiled as part of the boot behavior.

    let boot_stmts = [];
    flatten(crlf.ast).forEach(function (stmt) {
        if (is_definable(stmt)) {
            if (def_stmts[stmt.ptrn.ident] !== undefined) {
                error("defined_a", stmt, stmt.ptrn.ident);
            }
            def_stmts[stmt.ptrn.ident] = stmt;
        } else {
            boot_stmts.push(stmt);
        }
    });

// Statements that seemed pure at face value may not be, if they reference
// impure statements or ambient capabilities.

    (function cull() {
        let pure = new Set(
            Object.keys(primitives).concat(Object.keys(def_stmts))
        );
        if (Object.entries(def_stmts).some(function ([ident, stmt]) {
            const free = new Set(Object.keys(analyze_idents(stmt).free));
            if (free.isSubsetOf(pure)) {
                return false;
            }
            delete def_stmts[ident];
            boot_stmts.push(stmt);
            return true;
        })) {
            return cull();
...
// Compile the pure module definitions.

    Object.entries(def_stmts).forEach(function ([ident, def_stmt]) {
        definitions[ident] = eval_expr(def_stmt.expr);
    });

// Impure statements are compiled as a block capturing builtins in its
// environment.

    if (boot_stmts.length > 0) {
        env = {
            kind: "pair_ptrn",
            head: unflatten("pair_ptrn", [
                {kind: "ident_ptrn", ident: "random"},
                {kind: "ident_ptrn", ident: "timer"},
                {kind: "ident_ptrn", ident: "println"},
                {kind: "ident_ptrn", ident: "system"}
            ]),
            tail: env,
            depth: 0
        };
        const block = gen_block_code(unflatten("stmt_pair", boot_stmts));
        definitions.boot =
        ...
        requestorize(function () {
            ...
            while (true) {
                ...
                if (
                    ...
                    const message = ufork.fault_msg(
                        ufork.fix_to_i32(status)
                    );
...
function make_claims() {
    return {
        ...
        "scoped circular reference": bad("[DEF x AS x]"),
        ...
        `, "-1,+0,+1,#?"),
        "redefinition pure": bad(`
            DEF x AS 1
            DEF x AS 2
        `),
        "redefinition boot": bad(`
            DEF a AS 1
            DEF a AS println
        `),
        "scoped redefinition": bad(`
            [
                DEF x AS 1
                DEF x AS 2
            ]
        ...
        //     CREATE a WITH 1
        // `),
        "scoped redefinition of CREATE": bad(`
            [
                DEF a AS 1
                CREATE a WITH 1
            ]
        ...
        `, "#t"),
        "missing boot cap": good(`
            SEND system(777) TO println
        `, "#?"),
        "SEND to non cap": good(`
            SEND ? TO ?
            SEND TRUE TO println
        `, "#t"),
        "nested closures": good(`
            DEF a(p) AS [
                DEF b(q) AS [
                    DEF c(r) AS [
                        DEF d(s) AS [
            ...
            SEND 'p' TO NEW a
        `, "+113,+114"),
        "static definitions": good(`
            DEF x AS 666
            DEF y AS TRUE,FALSE,x
            DEF z(x) AS x(1)
        `, []),
        "mutual recursion": good(`
            DEF x(n) AS y(sub(n, 1))
            DEF y(n) AS CASE less(n, 10) OF
            TRUE : n
            FALSE : x(n)
            END
            SEND x(42) TO println
...
function test_compile(claims, log_level) {
    parseq.parallel(
...
if (import.meta.main) {
    ...
    test_path_contains();
    test_compact();
    const claims = make_claims();
    test_compile(claims, ufork.LOG_INFO); // LOG_INFO | LOG_WARN | LOG_DEBUG

// To run a particular test case, comment out the above call to 'test_compile'
// and uncomment the block below.

    // claims["forward reference"](function (report, reason) {
    //     if (report === undefined) {
    //         globalThis.console.log(reason);
    //     } else {
