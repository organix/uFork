...
// Compile the closure's code.
        ...
        const closure_return = ir_ref("return", "$hum");
        push_env(ast.ptrn, 0);
        if (def_ptrn?.kind === "ident_ptrn") {
            env.abs_label = label;
        }
        env.def_abs_ptrn = def_ptrn;
        env.return = closure_return;
...
// creation of a dynamic closure and jump straight to the code.
            ...
            if (ast.abs.kind === "ident_expr") {
                ...
                if (nth_imms !== undefined && nth_imms[0] < 0) {
                    ...
                    const code = ir_ref(nth(env, nth_imms[0] + 1).abs_label);
                    k =                                 // k env
                        ...
                        external(-2, self_tail)))));    // args k env'
                    return k;
                }
            }
            ...
            k =                                         // k env
                ...
                external(-1, tail_call)));              // args k
            return k;
        }

// Otherwise obtain the closure (or procedure) as a value and call it.
...
//debug         }, 1000);
//debug         run(text, log_level)(function (value, reason) {
//debug             if (value === undefined) {
//debug                 return console.log(reason);
//debug             }
...
//debug     DEF iterative(n, sum) AS iterative(sub(n, 1), add(sum, 1))
//debug     SEND iterative(50, 0) TO println
//debug `;
...
//debug check(ufork.LOG_DEBUG); // LOG_INFO | LOG_WARN | LOG_DEBUG | LOG_TRACE
