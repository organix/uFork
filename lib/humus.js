...
//debug //console.log(depths, op, imm, `(${delta})`);
    ...
    function ir_if_instr(make_t, make_f) {

// Both branches an 'if' instruction must accumulate the same stack depth
// adjustment, otherwise our scheme of predicting stack evolution in reverse is
// hopeless. We enforce this invariant by comparing stack depths before and
// after generating instructions for each branch.

        const initial = depths.slice();
        const t = make_t();
        const expected = depths.slice();
        depths = initial;
        const f = make_f();
        if (depths.join() !== expected.join() && errors.length === 0) {
...
// Unary primitive functions.
            ...
            if (ast.abs.ident === "neg" || ast.abs.ident === "not") {
                ...
                if (ast.abs.ident === "not") {
                    k = intern(k);
                    k = generate(ast.arg,
                        as(ast, ir_if_instr(push_false, push_true)));
                    return k;
                }
            }
                ...
                k = generate(ast.arg.head,
                    as(ast, ir_if_instr(
                        () => generate(ast.arg.tail,
                              as(ast, ir_if_instr(push_true, push_false))),
                        push_false
                    )));
                return k;
            }
