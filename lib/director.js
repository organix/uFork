...
// 63      | 3F    | ?     | undefined value
// 64      | 40    | @     | actor message/self
...
function tokenize(text, src) {
    ...
    function invalid(kind = "token") {
        while (true) {
            advance();
            if (code === undefined || is_punctuation(code)) {
                break;
            }
        }
...
function parse(text, src, top) {
    ...
    function head(t) {  // prefix operator
        if (
            ...
            // these tokens evaluate to themselves
            return t;
        }
        if (t.kind === "operator") {
