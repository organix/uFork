...
function parse(text, src, top) {
    ...
    let enhance;  // FORWARD DECLARATION
    function peek() {
        token = tokens.at(at);
        if (token === undefined) {
            token = end_token;
    ...
    function infix_ops(lbp, lsp = 1, rsp = 1) {
    ...
    operators[","] = base_ops;
    operators[":"] = base_ops;
    // operators[":"] = infix_ops(10, 0, 1);
    operators["=="] = infix_ops(40);
    operators["!="] = infix_ops(40);
    operators["<"] = infix_ops(40);
    operators["<="] = infix_ops(40);
    operators[">"] = infix_ops(40);
    operators[">="] = infix_ops(40);
    operators["+"] = infix_ops(50);
    operators["-"] = infix_ops(50);
    operators["*"] = infix_ops(60);
    operators["/"] = infix_ops(60);
    // operators["!"] = prefix_ops(70);
    ...
    operators["{"] = Object.assign(
        ...
        base_ops,
        {
            head: function nud(self) {
                ...
                while (!is_empty) {
/*
                    // FIXME: expect "key: value" instead?
                    const key_value = expression(0);
                    if ((key_value.kind !== "operator") || (key_value.text !== ":")) {
...
function test_parse() {
    ...
    //         },
    //         tokens: [
    //             {kind: "number", line: 1, column: 1, base: 10, exponent: 0, significand: 1, start: 0, end: 1, text: "1", lsp: 0, rsp: 0, lbp: 0},
    ...
    //                 {kind: "number", line: 1, column: 1, base: 10, exponent: 0, significand: 1, start: 0, end: 1, text: "1", lsp: 0, rsp: 0, lbp: 0},
    //                 {kind: "number", line: 1, column: 5, base: 10, exponent: 0, significand: 2, start: 4, end: 5, text: "2", lsp: 0, rsp: 0, lbp: 0}
    //             ]},
