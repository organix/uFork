...
function compile(text, src) {
    ...
    */
    ...
    function xlat_invoke(ctx, crlf, k) {
        ...
        if (equal_to(tail_pos_k, k)) {
            ...
            let code =
                ...
                new_instr(debug, "pair", nargs, // (cust args...)
                interpret(ctx, func,            // (cust args...) closure
                new_call_instr(debug, new_2,    // (cust args...) beh.(state)
                scm_send_msg(debug)))))));
            return code;
        } else {
    ...
    function xlat_BECOME(ctx, crlf, k) {
        ...
        let code =
            interpret(ctx, beh,                 // beh=[#behavior_t, code, data, meta]
            new_BEH_tag(debug, "beh", undef_lit,// --
            k));
        return code;
    }
